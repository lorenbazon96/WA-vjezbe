<template>
  ...
  >
    ...
    <div
      ...
      class="mt-4 max-w-2xl mx-auto max-h-40 overflow-y-auto bg-slate-800/50 rounded-lg p-3 border border-slate-600"
    >
        ...
        >
          ...
          <div class="flex items-center gap-3">
            <div class="text-orange-400 font-semibold">
              {{ izracunajCijenuStavke(stavka).toFixed(2) }}€
            </div>
    ...
    >
      <div class="bg-slate-800 rounded-xl p-6 w-full max-w-md border border-slate-600 relative">
        ...
        <div class="space-y-3">
          <div>
            <label class="text-sm text-slate-300">Ime</label>
            <input
              v-model="dostava.ime"
              type="text"
              placeholder="Ime"
            ...
            />
            <p v-if="errors.ime" class="text-red-400 text-xs mt-1">{{ errors.ime }}</p>
          </div>
...
function validateDostava() {
  let ok = true
  errors.value = { ime: '', adresa: '', telefon: '' }

  if (!dostava.value.ime.trim()) {
    errors.value.ime = 'Ime je obavezno'
    ok = false
  }
  ...
  const tel = String(dostava.value.telefon || '').trim()
  if (!tel) {
    errors.value.telefon = 'Telefon je obavezan'
    ok = false
  } else if (!/^\d+$/.test(tel)) {
    errors.value.telefon = 'Telefon smije sadržavati samo brojeve'
    ok = false
...
async function posaljiNarudzbu() {
  try {
    ...
    const payload = {
      ime: dostava.value.ime,
      adresa: dostava.value.adresa,
      telefon: dostava.value.telefon,
      narucene_pizze: narucene_pizze.value,
    }

    const odgovor = await axios.post('http://localhost:3000/narudzba', payload)
    ...
    dostava.value = { ime: '', adresa: '', telefon: '' }
    errors.value = { ime: '', adresa: '', telefon: '' }
