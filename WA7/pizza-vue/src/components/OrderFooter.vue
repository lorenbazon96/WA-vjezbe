...
async function posaljiNarudzbu() {
  try {
    ...
    if (!token) {
      statusPoruka.value = 'Morate se prijaviti prije narudžbe.'
    ...
    const odgovor = await api.post('/narudzba', payload)
    ...
    statusPoruka.value = error.response?.data?.message || 'Greška pri slanju narudžbe.'
    statusTip.value = 'error'
  }
}
</script>
